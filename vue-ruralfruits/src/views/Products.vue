<template>
  <div>
    ...
    <div class="box" style="display: flex;">
      ...
      <el-button type="success" icon="EditPen" @click="handleBatchEdit">修改</el-button>
    ...
    <div class="box">
      <el-table :data="data.tableData" style="width: 100%" @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="userId" label="所属用户ID" width="120" />
    ...
    <el-dialog v-model="visible" :title="title" width="900">
      <el-form :model="form" :rules="rules" ref="form1" :label-width="85">
        ...
        <el-form-item label="所属用户ID" prop="userId">
          <el-input v-model="form.userId" type="number" autocomplete="off" placeholder="请输入所属用户ID" />
        </el-form-item>
...
const form = ref({});
const form1 = ref();
const uploadUrl = ref(import.meta.env.VITE_API_BASE_URL + '/sysProducts/import');
const categoriesList = ref([]);
const isAdmin = true; // 管理员标记（关键）
...
const rules = reactive({
  ...
  userId: [{ required: true, message: '请输入所属用户ID', trigger: 'blur' }]
})
...
onMounted(() => {
  ...
  getProductsList(); // 无需登录校验，直接加载
})

// ========== 查询参数（含isAdmin=true，关键传递） ==========
const queryParams = reactive({
  pageNum: 1,
  pageSize: 5,
  name: '',
  category: '',
  isAdmin: isAdmin // 管理员标识，必须传递
})
...
const handleBatchDelete = () => {
  ...
  ElMessageBox.confirm(
      `是否确认删除选中的${ids.value.length}条商品？`,
      "删除确认",
      { type: "warning" }
  ).then(() => {
    // 传递isAdmin=true，无需token
    deleteProducts(ids.value.join(','), isAdmin).then(res => {
...
const handleEdit = (row) => {
  getProducts(row.id, isAdmin).then((res) => { // 传递isAdmin=true
    if (!res.data) {
...
// ========== 导出商品 ==========
const handleExport = () => {
  download("/sysProducts/export", { ...queryParams, isAdmin: isAdmin });
}
